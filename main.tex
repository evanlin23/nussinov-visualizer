...
For initial testing and verification, we plan to use synthetic data where the optimal secondary structure is known and predictable. To benchmark our algorithm implementation, we will generate random sequences at varying lengths to test performance and scalability.
\section*{Planned method/experimentations}
By using a heatmap (Base Pair Frequency Plot) of the solution space we can see which parts of solutions are more common which will hopefully lead to better predictions. We also plan on benchmarking our implementation by generating a synthetic dataset with random sequences of varying lengths.
